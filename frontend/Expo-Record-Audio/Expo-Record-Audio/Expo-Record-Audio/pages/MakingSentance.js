...
export default function MakingSentence({ navigation }) {
  ...
  useEffect(() => {
    ...
    const sentenceResulttRef = firebase.database().ref('sentenceResult');
    ...
    const onProcessDoneChange = async (snapshot) => {
      ...
      if (processDone) {
        try {
          const sentenceResultSnapshot = await sentenceResulttRef.once('value');
          const sentenceResult = sentenceResultSnapshot.val();
          console.log("sentence Result:", sentenceResult);

          if (sentenceResult === true) {
            Alert.alert("Success", "Processing was successful");
          } else if (sentenceResult === false) {
          ...
          await firebase.database().ref().update({ processDone: false, sentenceResult: null });
        } catch (error) {
  ...
  async function stopRecording() {
    ...
    try {
      ...
      try {
        ...
        const file = new File([blob], 'Recording2.3gp', { type: mimeType });

        const storageRef = firebase.storage().ref();
        const audioRef = storageRef.child('recordings/Recording2.3gp');
        ...
        const newSentenceRef = firebase.database().ref('newSentence');
        await newSentenceRef.set(true);

        
        // setTimeout(async () => {
        //   await newSentenceRef.remove();
        //   console.log('newSentence node removed');
        // }, 5000);
  ...
  return (
    <View style={styles.container}>
      <Image
        style={styles.image}
        source={require("./../images/microphone-xxl.png")}
      />
